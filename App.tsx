...
import { checkApiKey, promptApiKeySelection, generateResizedLayout } from './services/geminiService';
...
const App: React.FC = () => {
  ...
  const handleConnect = async () => {
    try {
      ...
      // Assume success as per instructions and re-verify immediately
      await verifyApiKey();
    } catch (e) {
  ...
  const handleGenerate = async () => {
    ...
    if (!apiKeyReady) {
      setError("Please connect your Google Cloud Project first.");
      return;
    }
    ...
    try {
      ...
      if (e.message === "API_KEY_MISSING") {
         ...
         setError("API Key verification failed. Please connect your project again.");
      } else {
    ...
    return (
        <div className="min-h-screen bg-zinc-950 flex flex-col items-center justify-center p-6 text-zinc-100 relative overflow-hidden">
            ...
            <div className="absolute top-0 left-0 w-full h-full overflow-hidden pointer-events-none">
                <div className="absolute top-1/4 left-1/4 w-96 h-96 bg-lime-500/10 rounded-full blur-[100px]"></div>
                <div className="absolute bottom-1/4 right-1/4 w-64 h-64 bg-zinc-700/20 rounded-full blur-[80px]"></div>
            </div>

            <div className="max-w-md w-full text-center space-y-8 relative z-10">
                <div className="mx-auto w-24 h-24 bg-zinc-900 rounded-3xl flex items-center justify-center border border-zinc-800 shadow-[0_0_30px_rgba(163,230,53,0.1)]">
                    <Sparkles className="w-12 h-12 text-lime-400" />
                </div>
                <div>
                    <h1 className="text-4xl font-bold tracking-tight mb-2 text-white">SmartLayout <span className="text-lime-400">Resizer</span></h1>
                    <p className="text-zinc-400 text-lg">AI-powered intelligent composition resizing for professional visuals.</p>
                </div>
                
                <div className="bg-zinc-900/50 border border-zinc-800 rounded-xl p-6 backdrop-blur-md">
                    <p className="text-sm text-zinc-300 mb-6 leading-relaxed">
                        To use the high-quality <span className="font-semibold text-lime-400">Nano Banana Pro</span> (Gemini 3 Pro) model, you need to connect a paid Google Cloud Project.
                    </p>
                    <Button 
                    ...
                    <p className="mt-4 text-xs text-zinc-600">
                        See <a href="https://ai.google.dev/gemini-api/docs/billing" target="_blank" rel="noreferrer" className="text-lime-400/80 hover:text-lime-400 hover:underline">Billing Documentation</a> for more details.
                    </p>
                </div>
            </div>
        </div>
